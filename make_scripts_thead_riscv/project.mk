...
#
# This makefile requires the environment variable BL_SDK_PATH to be set to the top-level bl60x_sdk directory
...
# make BL_SDK_PATH a "real" absolute path
...
SANITISED_BL_SDK_PATH:=$(realpath $(BL_SDK_PATH))
else
SANITISED_BL_SDK_PATH:=$(realpath $(wildcard $(BL_SDK_PATH)))
endif

export BL_SDK_PATH := $(SANITISED_BL_SDK_PATH)

ifndef BL_SDK_PATH
$(error BL_SDK_PATH variable is not set to a valid directory.)
endif

ifneq ("$(BL_SDK_PATH)","$(SANITISED_BL_SDK_PATH)")
$(error If BL_SDK_PATH is overriden on command line, it must be an absolute path with no embedded shell special characters)
endif

ifneq ("$(BL_SDK_PATH)","$(subst :,,$(BL_SDK_PATH))")
$(error BL_SDK_PATH cannot contain colons. If overriding BL_SDK_PATH on Windows, use MSYS Unix-style /c/dir instead of C:/dir)
endif
...
COMMON_MAKEFILES := $(abspath $(BL_SDK_PATH)/make_scripts_thead_riscv/project.mk $(BL_SDK_PATH)/make_scripts_thead_riscv/common.mk $(BL_SDK_PATH)/make_scripts_thead_riscv/component_wrapper.mk $(firstword $(MAKEFILE_LIST)))
...
COMPONENT_DIRS := $(PROJECT_PATH)/components $(EXTRA_COMPONENT_DIRS) $(BL_SDK_PATH)/components $(BL_SDK_PATH)/customer_components $(PROJECT_PATH)/$(PROJECT_NAME) $(PROJECT_COMPONENT)
endif
...
ifeq ("$(wildcard ${BL_SDK_PATH}/components.mk)","")
...
include $(BL_SDK_PATH)/components.mk
...
include $(BL_SDK_PATH)/make_scripts_thead_riscv/common.mk
include $(BL_SDK_PATH)/make_scripts_thead_riscv/toolchain.mk

all:
ifeq ("$(OS)","Windows_NT")
else
ifeq ("$(CONFIG_CHIP_NAME)", "BL602")
	#@cd $(BL_SDK_PATH)/image_conf; python3 flash_build.py $(PROJECT_NAME) $(CONFIG_CHIP_NAME)
	@cd $(BL_SDK_PATH)/image_conf; ./flash_build $(PROJECT_NAME) $(CONFIG_CHIP_NAME)
endif
endif
...
ifeq ("$(wildcard ${BL_SDK_PATH}/version.txt)","")
BL_SDK_VER := $(shell cd ${BL_SDK_PATH} && git describe --always --tags --dirty)
ifeq ("$(CONFIG_CHIP_NAME)", "BL602")
BL_SDK_PHY_VER := $(shell cd ${BL_SDK_PATH}/components/bl602/bl602_wifi/plf/refip/src/driver/phy/bl602_phy_rf/ && git describe --always --tags --dirty)
BL_SDK_RF_VER := $(shell cd ${BL_SDK_PATH}/components/bl602/bl602_wifi/plf/refip/src/driver/phy/bl602_phy_rf/rf && git describe --always --tags --dirty)
endif
$(info use git describe to generate version.txt)
else
BL_SDK_VER := `cat ${BL_SDK_PATH}/version.txt |head -n1`
ifeq ("$(CONFIG_CHIP_NAME)", "BL602")
BL_SDK_PHY_VER := `cat ${BL_SDK_PATH}/version.txt |head -n2|tail -n1`
BL_SDK_RF_VER := `cat ${BL_SDK_PATH}/version.txt |head -n3|tail -n1`
endif
...
+$(MAKE) -C $(BUILD_DIR_BASE)/$(2) -f $(BL_SDK_PATH)/make_scripts_thead_riscv/component_wrapper.mk COMPONENT_MAKEFILE=$(1)/bouffalo.mk COMPONENT_NAME=$(2)
endef
...
flash: all
	cd $(BL_SDK_PATH)/tools/flash_tool && env SDK_APP_BIN=$(APP_BIN) SDK_BOARD=$(PROJECT_BOARD) SDK_NAME=$(PROJECT_NAME) SDK_MEDIA_BIN=$(APP_MEDIA_BIN) SDK_ROMFS_DIR=$(APP_ROMFS_DIR) SDK_DTS=$(PROJECT_DTS) SDK_XTAL=$(PROJECT_BOARD_XTAL) BL_FLASH_TOOL_INPUT_PATH_cfg2_bin_input=$(APP_BIN) python3 core/bl60x_simple_flasher.py bl602 bl602/conf/iot.toml
