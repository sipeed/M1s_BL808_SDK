...
// partion media
#define LFS_OFFSET_IN_FLASH (0x300000 + 0x1000)
                              ...
                              lfs_size_t size)
{
    ...
    return bl_flash_read(LFS_OFFSET_IN_FLASH + block * cfg->block_size + off, (uint32_t)buffer, size)
               ? LFS_ERR_IO
               : LFS_ERR_OK;
}
                              ...
                              lfs_size_t size)
{
    ...
    LFS_ASSERT(block < cfg->block_count);
    return bl_flash_write(LFS_OFFSET_IN_FLASH + block * cfg->block_size + off, (uint32_t)buffer, size)
               ? LFS_ERR_IO
               : LFS_ERR_OK;
}
...
static int lfs_spi_flash_erase(const struct lfs_config *cfg, lfs_block_t block)
{
    ...
    LFS_ASSERT(block < cfg->block_count);
    return bl_flash_erase(LFS_OFFSET_IN_FLASH + block * cfg->block_size, cfg->block_size) ? LFS_ERR_IO
                                                                                                : LFS_ERR_OK;
}
...
static int32_t lfs_vfs_rename(file_t *fp, const char *oldpath, const char *newpath)
{
    ...
    if (ret < LFS_ERR_OK) LFS_DEBUG("%s %s->%s return %d", __func__, oldpath, newpath, (int)ret);
    ret = lfs_ret_value_convert(ret);
...
static aos_dir_t *lfs_vfs_opendir(file_t *fp, const char *path)
{
    ...
    if (ret < LFS_ERR_OK) LFS_DEBUG("%s return %d", __func__, (int)ret);
    ret = lfs_ret_value_convert(ret);
    if (ret < 0) {
#ifdef CONFIG_VFS_USE_ERRNO
        errno = -ret;
...
static int32_t lfs_vfs_mkdir(file_t *fp, const char *path)
{
    ...
    if ((ret < LFS_ERR_OK) && (ret != LFS_ERR_EXIST)) {
        LFS_DEBUG("%s %s return %d", __func__, path, (int)ret);
    }
...
static int32_t lfs_vfs_rmdir(file_t *fp, const char *path)
{
    ...
    if (ret < LFS_ERR_OK) LFS_DEBUG("%s %s return %d", __func__, path, (int)ret);
    ret = lfs_ret_value_convert(ret);
